...
where Model: ModelProtocol
{
    ...
    /// Publisher for all state updates generated by actions
    ...
    public init(
        ...
        self.cancelTransactions = updates
    ...
    /// Generate next state and effects for action
    private func update(_ action: Model.Action) -> Model.Update {
        Model.update(
            state: self.state,
            action: action,
    ...
    /// Perform state transaction given update
    private func transact(_ next: Model.Update) {
